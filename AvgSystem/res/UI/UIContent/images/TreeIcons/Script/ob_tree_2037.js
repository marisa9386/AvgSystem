...
function ob_rsc(ob_xmlhttp2, ob_url2, ob_tb2, callback)
{
  if(ob_xmlhttp2.readyState==4)
  {
	...
	//check the status of the call, and run a user function if defined
	if ( ob_xmlhttp2.status != '200' )
	{
		if ( typeof(ess_ob_rsc_error) == 'function' )
		{
			ess_ob_rsc_error();
			return;
		}
    ...
    if(ob_xmlhttp2.responseText)
    {
      ...
      if(tempDiv.childNodes[2].firstChild) {if(typeof(ob_tnodrag) == "function") 
      {
  ...
  if(ob_tb2.innerHTML == "Loading ...")
  {
    ...
    oTempLoadingNode=ob_tb2.parentNode.parentNode.parentNode;oTempLoadingNode.parentNode.removeChild(oTempLoadingNode);
  }
...
{
  if ( typeof(ess_ob_t24) == 'function' )
  {
    ess_ob_t24(node, url, callback);
    return;
  }
